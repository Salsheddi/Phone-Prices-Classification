...
Based on the analysis of the operating systems graph, it is clear that the "OS" column in our dataset contains various forms of noise. Some phones have unspecified OS values such as "Android," while others have improperly formatted entries like "Android 10 / Android 11" or "Android 12 or 13." Additionally, certain phone operating systems, like "Tizen 3.0," and "Android 6.0.1" are exceptionally rare in our dataset, with only a single occurrence. To mitigate potential bias in our classification model and enhance its accuracy, we have decided to remove rows associated with these problematic OS values.



```{r}
library(dplyr)
dataset_phones <- dataset_phones %>% filter(dataset_phones$os != "Tizen 3.0")
dataset_phones <- dataset_phones %>% filter(dataset_phones$os != "iOS 11")
dataset_phones <- dataset_phones %>% filter(dataset_phones$os != "iOS 15")
dataset_phones <- dataset_phones %>% filter(dataset_phones$os != "Android 7.0.1")
dataset_phones <- dataset_phones %>% filter(dataset_phones$os != "Android 6.0.1")
dataset_phones <- dataset_phones %>% filter(dataset_phones$os != "Android 12L")
dataset_phones <- dataset_phones %>% filter(dataset_phones$os != "iOS 15.4")
dataset_phones <- dataset_phones %>% filter(dataset_phones$os != "iOS 15.4")
dataset_phones <- dataset_phones %>% filter(dataset_phones$os != "Android")
dataset_phones <- dataset_phones %>% filter(dataset_phones$os != "Android 12 or 13")
dataset_phones <- dataset_phones %>% filter(dataset_phones$os != "Android 10/ Android 11")
dataset_phones <- dataset_phones %>% filter(dataset_phones$os != "iOS 11.1.1")
dataset_phones <- dataset_phones %>% filter(dataset_phones$os != "EMUI 13")
dataset_phones <- dataset_phones %>% filter(dataset_phones$os != "Android 5.1")
dataset_phones <- dataset_phones %>% filter(dataset_phones$os != "Android 8.0 Oreo")
dataset_phones <- dataset_phones %>% filter(dataset_phones$os != "Android 6")
dataset_phones <- dataset_phones %>% filter(dataset_phones$os != "Android 9.0 Pie")
dataset_phones <- dataset_phones %>% filter(dataset_phones$os != "iOS 12")
...
it has a lower  value but it still contributes in the price so no need to delete the attributes.
