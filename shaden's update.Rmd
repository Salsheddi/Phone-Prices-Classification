...
When we calculate and plot the average silhouette, we see that both cluster's have positive values but the second one has little negative values in it which indicates that part of the second cluster doesn't fit well within any cluster and may have distinct characteristics or patterns. 
...
```{r}
dataset_phones_3 <- scale(dataset_phones_3)
 km <- kmeans(dataset_phones_3, 9, iter.max = 140 , algorithm="Lloyd", nstart=100) 
 km
#total within-cluster-sum of square
twss <- sum(km$withinss)
print(twss)
```
By applying the k-means algorithm with a cluster value of 9, we generated the third size k cluster. After that, we calculated the total within-cluster sum of squares (TWSS), which has a value of 10572.95 The objective of the k-means algorithm is to minimize the TWSS, and in this case, the algorithm successfully achieved the desired goal since the TWSS is the smallest compared to other clusters with different values of k.
...
After calculating and plotting the average silhouette, we can see that the majority of the clusters have positive values. However, there are four clusters with negative values, but with very few values in each of them which indicating that they do not fit well within any cluster and may have distinct characteristics or patterns.
...
In summary, the clusters are varying in size and display significant overlap. However, there is considerable diversity in the distances within the clusters, This is mostly in the middle clusters.

The largest cluster has a maximum size of 294, while the smallest cluster has a minimum size of 2.
...
```
Approximately 80.1% of the instances assigned to the same cluster are truly linked or similar to each other, according to the BCubed Precision score of 0.8010014  This implies that the algorithm shows a good degree of accuracy in correctly recognizing instances that are similar within clusters. Having the highest precision out of the rest of the cluster sizes meaning that it may have better precision in identifying positive instances

However, the average number of comparable or related instances inside each true cluster that is correctly assigned to the same cluster is only 18.18%, as indicated by the BCubed Recall score of 0.181862  This suggests that the recall of actually linked points inside clusters is poor.


```{r}
library(NbClust)
#a)fviz_nbclust() with silhouette method using library(factoextra) 
fviz_nbclust(dataset_phones_3, kmeans, method = "silhouette")+labs(subtitle ="Silhouette method")

```
After reviewing the Silhouette method's output from the plot above, we may say that 9 clusters are the appropriate and most suitable number of cluster for our data set. Even though clusters of k value 9 happen to be the clusters with the most overlap and lowest value of recall, it also happens to be the most fitting in regards to precision, TWSS and average silhouette width (as seen above in the plot). Lastly, because the overlap in k value 9 is somewhat considered high and our dataset is considered unbalanced, it could mean that clustering isn't a good option for our specific dataset, as unbalanced datasets can result to biased or misleading outcomes.
