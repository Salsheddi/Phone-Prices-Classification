...
```
The root of the decision tree is the "width" feature, which has the highest information gain. This means that the model, during training, found "width" to be the most significant feature for splitting and making predictions from confusion matrix shows that the model has correctly predicted all instances of the "cheap" class (True Positives = 275) but has failed to correctly identify any instances of the "affordable" and "expensive" classes (True Positives = 0 for both) Sensitivity (True Positive Rate) for the "cheap" class is 1.0, indicating that the model correctly identifies all instances of "cheap" phones in the test data The overall accuracy is 85.14%, but its misleading due to the class imbalance or it could be due to the method we chossed to partition our dataset the tarining data didnt include any expensive and affordable phones for the classifier to learn them
...
```{r}
pred.prune = predict(pruned.tree, testData, type="class")
table(pred.prune, testData$price)
```
...
```{r}
Phones.S <- dataset_phones_2[,-12]

set.seed(234)
train = sample(1:nrow(Phones.S), 200)
Phones.Strain=Phones.S[train,]
Phones.Stest=Phones.S[-train,]
```

```{r}
fittree = rpart(price ~ ., data=Phones.Strain, method="anova", cp=0.008)
fittree
```
