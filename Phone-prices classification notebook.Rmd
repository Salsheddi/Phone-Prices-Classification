...
precision_affordable <- 8 / (8 + 1)
precision_cheap <- 274 / (274 + 1)
...
The model's performance was evaluated using sensitivity and specificity values for each class. The "affordable" class had a sensitivity of 22.85%, indicating only 22.85% of positive instances were correctly identified(True Positives = 8).The model identified 98.95% of negative instances in this category.
...
With a precision of 0.88,the model exhibited high precision in identifying positive instances in the "affordable" class. The model demonstrates strong precision for the "cheap" of 0.99, indicating its effectiveness in precisely identifying positive instances. On the other hand, the NaN accuracy value of the "expensive" class indicated that no positive examples were anticipated.
...
```{r}
# Precision calculations
precision_affordable <- 9 / (9 + 9)
precision_cheap <- 266 / (266 + 9)
precision_expensive <- 0 / (0 + 0)
...
The model's precision in identifying true positives is moderate in the "affordable" class, with only half of predicted instances being true positives. However, the "cheap" class has a high precision of 0.967, with approximately 96.73% of instances predicted as "cheap" being true positives. The model's strong performance in classifying cheap instances is evident, but the precision for the "expensive" class is NaN, indicating potential limitations or insufficient data. 

In summary, the decision tree based on the Gini index faced similar challenges to the decision tree based on information gain when it came to accurately classifying instances in the "affordable" and "cheap" classes. The model's performance varied in terms of sensitivity, specificity and precision for each class, highlighting its strengths and weaknesses in differentiating between positive and negative instances.
...
```{r}
# Precision calculations
precision_affordable <- 4 / (4 + 0)
precision_cheap <- 191 / (191 + 0)
precision_expensive <- 0 / (0 + 1)
...
While high precision values for "affordable" and "cheap" suggest good performance in terms of making correct positive predictions for these classes, the 0% precision for "expensive" indicates that the model is not making correct positive predictions for this class.

The decision tree demonstrates reasonably good overall accuracy but faces challenges in correctly classifying instances for certain classes. The values for specificity, sensitivity and precision provide insight into how well the model performs for each class.
...
```{r}
# Precision calculations
precision_affordable <- 4 / (4 + 2)
precision_cheap <- 189 / (189 + 5)
precision_expensive <- 0 / (0 + 0)
...
The precision analysis of the tree model reveals varying performance across different classes. The model's precision for predicting "affordable" is approximately 66.7% accuracy, indicating that two-thirds of predictions are accurate. In contrast the precision of for predicting "cheap" is high at 97.4%, indicating its effectiveness in accurately identifying instances labeled as "cheap." However, the precision for predicting "expensive" is undefined, suggesting no true positives for this class. The model performs well in terms of precision for "cheap," but there are issues with precision for "affordable,"
...
```{r}
# Precision calculations
precision_affordable <- 3/ (3 + 0)
precision_cheap <- 110 / (110 + 3)
precision_expensive <- 0 / (0 + 0)
...
The model's precision in predicting "affordable" is 66.7%, with two-thirds of predictions being accurate. Its precision in predicting "cheap" is high at 97.4%, indicating its effectiveness in identifying "cheap" instances. However, the precision for predicting "expensive" is undefined, indicating no true positives. 

In summary, the 90-10 split used for training and testing the model has provided valuable insights into its performance. While the decision tree faced challenges in classifying the underrepresented "expensive" class due to dataset imbalance, it still achieved a high overall accuracy. By analyzing the model's sensitivity, specificity and precision for each class, we can gain a deeper understanding of its strengths and weaknesses.
...
```{r}
# Precision calculations
precision_affordable <- 5/ (5 + 8)
precision_cheap <- 105/ (105 + 5)
precision_expensive <- 2 / (2 + 1)
...
The precision for the "affordable" class is 0.3846, indicating that only around 38.46% of instances predicted as "affordable" are true positives. This suggests that the model has a relatively low precision in correctly identifying positive instances within the "affordable" category. On the other hand, the precision for the "cheap" class is 0.9545, indicating a high level of precision in accurately identifying positive instances within this class, with approximately 95.45% precision. For the "expensive" class, the precision is 0.6667, suggesting that around 66.67% of instances predicted as "expensive" are true positives. While this precision is moderate, it indicates that the model has some ability to correctly identify positive instances within the "expensive" category. 
...
We created the first size k cluster with k-mean algorithm using k=2, we also calculated the total within-cluster-sum of square(TWSS) which in this case equals 15501.08
...
From the plot, we are able to determine that certain data points do not belong to any specific cluster, which makes them outliers. These outliers indicate something unusual or abnormal in the data. As well as, one cluster is noticeably larger than the other. 
...
The max size for the biggest cluster is 670, while the min size for the smallest cluster is 205, which indicates that the difference is big between values.
...
print(twss)
```
After applying the k-means algorithm with a cluster value of 4, we created the second size k cluster. When calculating the total within-cluster sum of squares (TWSS), we got a value of 12467.38 The objective of the k-means algorithm is to minimize the TWSS. In this case, the algorithm did not achieve the desired goal as the TWSS remains relatively high compared to other clusters with different values of k.
...
The largest cluster has a maximum size of 366, while the smallest cluster has a minimum size of 97	.
...
```
According to the BCubed Precision score of 0.8059296, there is an average of about 80.59% similarity or relatedness between the instances that were assigned to the same cluster. This implies that the algorithm shows good levels of accuracy in identifying instances that are similar within clusters. Having the highest precision out of the rest of the cluster sizes meaning that it may have better precision in identifying positive instances

On the other hand, the BCubed Recall score of 0.3093893 indicates that, within each genuine cluster, only approximately 30.93% of similar or related instances are correctly allocated to the same cluster on average. This suggests that the recall inside clusters is poor.
...
print(twss)
```
By applying the k-means algorithm with a cluster value of 6, we generated the third size k cluster. After that, we calculated the total within-cluster sum of squares (TWSS), which has a value of 10846.39 The objective of the k-means algorithm is to minimize the TWSS, and in this case, the algorithm successfully achieved the desired goal since the TWSS is the smallest compared to other clusters with different values of k.
...
fviz_silhouette(avg_sil)
```
From our analysis, we have identified certain data points that do not belong to any specific cluster, indicating outliers that may represent unusual or abnormal occurrences. Moreover, the varying shapes of the clusters suggest that they are not all of the same size.

After calculating and plotting the average silhouette, we can see that the majority of the clusters have positive values. However, there are three clusters with negative values, but with very few values in each of them which indicating that they do not fit well within any cluster and may have distinct characteristics or patterns.

Additionally, there's noticeable variation in distances within the clusters. Certain clusters seem to have closely packed data points, indicating a high degree of compactness, while others have more dispersed data points, leading to larger distances between them.

In summary, the clusters are varying in size and display overlap. However, there is considerable diversity in the distances within the clusters.

The largest cluster has a maximum size of 307, while the smallest cluster has a minimum size of 31.
...
```
Approximately 80.03% of the instances assigned to the same cluster are truly linked or similar to each other, according to the BCubed Precision score of 0.800386 This implies that the algorithm shows a good degree of accuracy in correctly recognizing instances that are similar within clusters. 

However, the average number of comparable or related instances inside each true cluster that is correctly assigned to the same cluster is only 23.71%, as indicated by the BCubed Recall score of 0.2371112 This suggests that the recall of actually linked points inside clusters is very poor and the lowest value of recall out of the rest of the cluster sizes.
...
```
After reviewing the Elbow method's output from the plot above, we may say that 4 clusters are the appropriate and most suitable number of cluster for our data set. Even though clusters of k value 4 happen to be the clusters with the most overlap and lowest value of recall, it also happens to be the most fitting in regards to percision and Elbow method output (as seen above in the plot). Lastly, because the overlap in k value 4 is somewhat considered high and TWSS was the best in k value 6 as well as, our dataset is considered unbalanced, it could mean that clustering isn't a good option for our specific dataset, as unbalanced datasets can result to biased or misleading outcomes.
