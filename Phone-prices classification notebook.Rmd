...
We created the first size k cluster with k-mean algorithm using k=2, we also calculated the total within-cluster-sum of square(TWSS) which in this case equals 6850.703
...
print(twss)
```
After applying the k-means algorithm with a cluster value of 4, we created the second size k cluster. When calculating the total within-cluster sum of squares (TWSS), we got a value of 4802.346 The objective of the k-means algorithm is to minimize the TWSS. In this case, the algorithm did not achieve the desired goal as the TWSS remains relatively high compared to other clusters with different values of k.
...
The largest cluster has a maximum size of 544, while the smallest cluster has a minimum size of 65	.
...
```
According to the BCubed Precision score of 0.8032289 , there is an average of about 80.3% similarity or relatedness between the instances that were assigned to the same cluster. This implies that the algorithm shows good levels of accuracy in identifying instances that are similar within clusters. Having the highest precision out of the rest of the cluster sizes meaning that it may have better precision in identifying positive instances

On the other hand, the BCubed Recall score of 0.4549247  indicates that, within each genuine cluster, only approximately 45.49% of similar or related instances are correctly allocated to the same cluster on average. This suggests that the recall inside clusters is poor.
...
In summary, the clusters are varying in size and display some overlap. However, there is considerable diversity in the distances within the clusters.

The largest cluster has a maximum size of 280, while the smallest cluster has a minimum size of 17.
...
```
Approximately 80.23% of the instances assigned to the same cluster are truly linked or similar to each other, according to the BCubed Precision score of 0.8023584  This implies that the algorithm shows a good degree of accuracy in correctly recognizing instances that are similar within clusters. 

However, the average number of comparable or related instances inside each true cluster that is correctly assigned to the same cluster is only 24.72%, as indicated by the BCubed Recall score of 0.2472029 This suggests that the recall of actually linked points inside clusters is very poor and the lowest value of recall out of the rest of the cluster sizes.
...
```
After reviewing the Elbow method's output from the plot above, we may say that 4 clusters are the appropriate and most suitable number of cluster for our data set. Even though clusters of k value 4 happen to be the clusters with the most overlap and lowest value of recall, it also happens to be the most fitting in regards to TWSS and Elbow method output (as seen above in the plot). Lastly, because the overlap in k value 4 is somewhat considered high and precision was highest in k value 6 as well as our dataset is considered unbalanced, it could mean that clustering isn't a good option for our specific dataset, as unbalanced datasets can result to biased or misleading outcomes.
